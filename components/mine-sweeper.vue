<template>
  ...
  <div :class='$style.grid' ref='gridRef'>
    <button 
      v-for='cell in cells' 
      :key='cell' 
      :class='[$style.cell, cell.opened && cell.mine && $style._mine, cell.opened && $style._opened, cell.flaged && $style._flaged]'
  ...
  <div v-if='cells.filter(c => !c.mine).every(c => c.opened)' :class='$style.win' @click='regenerateBoard'>
    <div>WIN!</div>
  </div>
</template>
